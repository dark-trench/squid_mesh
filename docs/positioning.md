...
| Replay | Supported, legacy | Replay remains a table-runtime API until its journal implementation lands. Under the journal default it returns an explicit unsupported-runtime error instead of reading legacy tables. |
...
| Jido.Storage-backed core | Supported, evolving | Start, execution, cancellation, inspection, explanation, pause, and approval controls run through the Jido journal runtime by default and do not write legacy runtime rows for the covered journal path. |
